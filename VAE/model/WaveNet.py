...
from model.ops import causal_conv
...
class WaveNetModel(object):
    ...
    def __init__(self,
        ...
        with tf.variable_scope(name): #本行以及其下一行都是自己加的
            ...
            self._create_network(obs_shape=obs_shape, embedding_shape=embedding_shape)
        ...
        with tf.variable_scope('wavenet'):
            ...
            with tf.variable_scope('causal_layer'):
                ...
                if self.scalar_input:
                    initial_channels = obs_shape
                    initial_filter_width = self.initial_filter_width
                else:
                    initial_channels = obs_shape # self.quantization_channels
                    ...
                    with tf.variable_scope('layer{}'.format(i)):
                        ...
                        if self.global_condition_channels is not None:
                            ...
                            current['gc_filtweights'] = create_variable(
                                'gc_filter',
                                [1, self.global_condition_channels,
                                 self.dilation_channels]) ##这个地方居然是1
    ...
    def _create_causal_layer(self, input_batch):
        ...
        '''
        with tf.name_scope('causal_layer'): #input_batch : [1, 63, 1]
            weights_filter = self.variables['causal_layer']['filter'] #[2, 1, 32]
    ...
    def _create_dilation_layer(self, input_batch, layer_index, dilation,
        ...
        weights_filter = variables['filter'] #[2, 32, 32]
        ...
        if global_condition_batch is not None:
            weights_gc_filter = variables['gc_filtweights'] # [1, 30, 32]
        ...
        skip_cut = out.shape[1] - output_width
        out_skip = tf.slice(out, [0, skip_cut.value, 0], [-1, -1, -1])
        weights_skip = variables['skip'] ##输出的512通道数是不是太大了！！！
